...
def main(stock_code,stock_name):
    try:
    ...
    if result[0].index.max() > (datetime.datetime.now() - datetime.timedelta(200)) and result[1]>1:
        ...
        wb=openpyxl.load_workbook(r'C:\Users\14834\Desktop\stock2\Share_data_xl\2022-12-17(选择code).xlsx')
        sheet=wb[wb.sheetnames[0]]
        sheet.append((stock_code,stock_name,result[1]))
        wb.save(r'C:\Users\14834\Desktop\stock2\Share_data_xl\2022-12-17(选择code).xlsx')
    else:
        pass
...
def designPlot(df, buyline, save_path, *q_list: list):
    ...
    try:
        plt.xticks(ticks=pd.date_range(df.index[0], df.index[-1] + datetime.timedelta(days=40), freq="M")[::4])
    except:
        pass
    plt.xlabel('日期')
    ...
    for item in buyline:
        plt.vlines(x=item, ymin=0, ymax=df.iloc[:, :1].values.max(), linewidth=0.5, linestyles="dashed")
...
if __name__ == '__main__':
    df600 = pd.read_excel(r'Share_data_xl\2022-12-17.xlsx', engine='openpyxl',
                          dtype='str')

    code_and_name = df600.iloc[:, :2].values   # 获取代码以及名称
    for item in code_and_name[:]:
